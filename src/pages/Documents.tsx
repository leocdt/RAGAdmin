...
const Documents: React.FC = () => {
  ...
  const fetchDocuments = async () => {
    try {
      const response = await axios.get('http://192.168.137.2:8000/api/documents/');
  ...
  const handleDelete = async (id: string) => {
    setLoading(true);
    try {
      await axios.delete(`http://192.168.137.2:8000/api/documents/${id}/`);
  ...
  const handleView = async (id: string) => {
    setLoading(true);
    try {
      const response = await axios.get(`http://192.168.137.2:8000/api/documents/${id}/content/`);
